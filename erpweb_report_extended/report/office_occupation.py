...
class OfficeOccupation(models.Model):
    _name = "erpweb.office.occupation.report"
    _description = "Office Occupation Report"
    _auto = False
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        res = super(OfficeOccupation, self).fields_get(allfields=allfields, attributes=attributes)
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        result = super(OfficeOccupation, self).read_group(
            domain, fields, groupby, offset=offset,
            limit=limit, orderby=orderby, lazy=lazy)
        products = self.env['product.product'].search([('product_category', '=', '2_office')])
        ...
        for r in result :
            if r.get('date:month', False):
                ...
                total_sol = sale_order_lines.filtered(lambda s: s.start_date.month == dt.month and s.product_id.product_category in ('2_office', '3_boardroom'))
    ...
    def _query(self):
        return """
            (SELECT %s
            FROM %s
            WHERE sol.is_rental AND pt.product_category = '2_office' AND p.active IS TRUE AND s.state in ('sale', 'done', 'sent')
            GROUP BY %s)
        """ % (
            self._select(),
            self._from(),
