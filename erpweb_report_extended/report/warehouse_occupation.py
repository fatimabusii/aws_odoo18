...
class WarehouseOccupation(models.Model):
    _name = "erpweb.warehouse.occupation.report"
    _description = "Warehouse Occupation Report"
    _auto = False
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        res = super(WarehouseOccupation, self).fields_get(allfields=allfields, attributes=attributes)
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        result = super(WarehouseOccupation, self).read_group(
            domain, fields, groupby, offset=offset,
            limit=limit, orderby=orderby, lazy=lazy)
        products = self.env['product.product'].search([('product_category', '=', '1_storage_unit')])
    ...
    def _query(self):
        return """
            (SELECT %s
            FROM %s
            WHERE sol.is_rental AND pt.product_category = '1_storage_unit' AND p.active IS TRUE AND s.state in ('sale', 'done', 'sent')
            GROUP BY %s)
        """ % (
            self._select(),
            self._from(),
